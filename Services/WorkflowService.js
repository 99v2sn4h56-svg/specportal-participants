...
const WorkflowService = (() => {
  ...
  function initialize() {
    ...
    registerAction("task.create", context => { const task = TaskService.create({ title: context.definition.name, assignedUser: context.inputs.assignedUser || UserContextService.getEmail(), dueDate: context.inputs.dueDate || "", priority: context.inputs.priority || "Medium", module: context.definition.owner, entity: { type: context.definition.entity, id: context.inputs.eventId || context.inputs.sessionId || context.inputs.studentKey || "" }, relatedEvent: context.inputs.eventId || context.inputs.sessionId || "", notes: context.inputs.notes || "", workflowExecutionId: context.execution.id }); context.outputs.taskId = task.id; return task; });
    registerAction("notification.queue", context => { const notification = NotificationService.queue({ channel: "In-App", target: context.inputs.target || context.inputs.assignedUser || UserContextService.getEmail(), title: context.definition.name, body: context.inputs.message || context.definition.description, workflowExecutionId: context.execution.id }); context.outputs.notificationIds = (context.outputs.notificationIds || []).concat(notification.id); return notification; });
  ...
  function requireCapabilities_(capabilities) { const missing = (capabilities || []).filter(capability => !UserContextService.hasCapability(capability)); if (missing.length) throw new Error(`Workflow capabilities required: ${missing.join(", ")}.`); }
