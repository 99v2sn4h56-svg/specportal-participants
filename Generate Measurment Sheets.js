const CONFIG = {
  ...
  // Kept in sync with COSTUME_CONFIG.allowedItems in
  // "Update Measurement Sheets.js" -- this list had drifted out of date
  // (was missing Freestyler / I Need A Dollar / Original (Smash) /
  // Underneath the Radar, and had a stale "Golden/Original" / "INTERVAL"
  // that don't match any real item name), so schools doing those items
  // would silently get no costume workbook generated by this tool.
  allowedItems: [
    'Alive',
    'Back in Black',
    'Better',
...
function generateCostumeSheets() {
  ...
  const grouped = {};
  const allowedItemsNormalised = CONFIG.allowedItems.map(normaliseText);
  ...
  data.forEach(row => {
    ...
    if (!item || !allowedItemsNormalised.includes(normaliseText(item))) return;
  ...
  if (!itemNames.length) {
    ui.alert(`No matching costume groups were found in the "${CONFIG.itemColHeader}" column.`);
    return;
  }
...
// safeSheetName() / cleanFileName() are defined once, in
// "Update Measurement Sheets.js" -- confirmed by inspecting the live Drive
// folder that its version of cleanFileName() is the one already producing
// the actual workbook file names (e.g. "Golden The Original - Costume
// Measurement Sheet"). Do not redeclare them here: Apps Script has one
// global scope across all files, so two same-named function declarations
// silently collide, with no reliable way to know which one "wins".
